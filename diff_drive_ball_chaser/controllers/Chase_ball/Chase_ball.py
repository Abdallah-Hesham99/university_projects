...
while robot.step(timestep) != -1:
  ...
  #  pid.setpoint = 3.14 if c >20 else 0
    ...
    print('out is {}, PID setpoint is {}'.format(out,pid.setpoint))
...
#    c += 1
    ...
    if (key==ord('Y')):
        ...
        #num_path = 'D:\My_files\Learning\Webots\Trial\Trial\data\pid_data.npy'
        #set_path = 'D:\My_files\Learning\Webots\Trial\Trial\data\setpoint2_data.npy'
       
      #  np.save(num_path,positions)
       # np.save(set_path,pids)
        pth = 'D:\My_files\Learning\Webots\Trial\Trial\Photos\pid_' + str(counter) +'.png'
        plt.savefig(pth)
        #plt.title(str(pid.Kd))
        #pid.Kd +=0.005
        counter +=1
